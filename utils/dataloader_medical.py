...
class UnetDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # jpg_path, png_path = self.annotation_lines[index].strip().split(' ')

        jpg_path, png_path =  self.annotation_lines[index],  self.annotation_lines[index].replace('PNGImages','mask')
        label_name         = png_path.rsplit('.', 1)
        png_path = label_name[0] + '_mask.' + label_name[1]

        # jpg_path = os.path.join(jpg_path)
        # png_path = os.path.join(png_path)
        #-------------------------------#
        #   从文件中读取图像
        #-------------------------------#
